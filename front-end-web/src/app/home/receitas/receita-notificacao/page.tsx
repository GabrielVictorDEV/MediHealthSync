...
const URL = String(configs.RECEITA_NOTIFICACAO)
...
const schema = z.object({
  ...
  numeroNotificacao: z
    .string()
    .min(5, { message: 'O campo número da notificação é obrigatório' }),
})

export default function ReceitaNotificacao() {
            ...
            <div className="flex flex-col items-center gap-2 ">
              <section className="flex flex-row gap-4">
                <div className="flex flex-col justify-between">
                  <label
                    htmlFor="numeroNotificacao"
                    className="text-lg px-2 pb-2"
                  >
                    Número dá notificação:
                  </label>
                  <input
                    id="numeroNotificacao"
                    placeholder="•••••••••"
                    name="numeroNotificacao"
                    type="text"
                    value={formData.numeroNotificacao}
                    onChange={handleChange}
                    onFocus={handleFocus}
                    className={`w-[290px] h-8 p-4 rounded-md focus:border-blue-500 border border-gray-300 outline-none ${
                      formErrors.numeroNotificacao ? 'border-red-500' : ''
                    }`}
                  />
                  {formErrors.numeroNotificacao ? (
                    <span className="text-red-500">
                      {formErrors.numeroNotificacao}
                    </span>
                  ) : (
